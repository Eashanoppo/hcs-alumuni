...
import { ImageIcon, Plus, Trash2, Calendar, LayoutDashboard, Loader2, Camera, ArrowLeft, Image as ImageIconLucide } from "lucide-react"
import Link from "next/link"
import { adminCreateGalleryPhoto, adminDeleteGalleryPhoto } from "@/app/actions/admin"
  ...
  return (
    <div className="min-h-screen bg-[#FAFAF7] p-8 md:p-12">
      <header className="flex items-center gap-6 mb-12">
        ...
        <div>
          <h1 className="text-3xl font-black text-primary tracking-tight">Photo Gallery Management</h1>
          <p className="text-muted text-sm font-medium">Manage memories and event captures for the alumni portal.</p>
        </div>
      </header>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] shadow-premium border border-gray-100 sticky top-12">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                 <label className="text-[10px] font-black text-muted uppercase tracking-widest block ml-2">Select Photo</label>
                 <div className="relative border-2 border-dashed border-gray-100 rounded-[2rem] p-4 text-center hover:border-primary transition-all cursor-pointer bg-[#FAFAF7] group h-[200px] flex items-center justify-center overflow-hidden">
                    <input type="file" required onChange={(e) => setFile(e.target.files?.[0] || null)} className="absolute inset-0 opacity-0 cursor-pointer z-20" accept="image/*" />
                    {file ? (
                      <img src={URL.createObjectURL(file)} alt="Preview" className="absolute inset-0 w-full h-full object-cover z-10" />
                    ) : (
                      <div className="flex flex-col items-center gap-2">
                         <Camera className="text-muted group-hover:text-primary" size={32} />
                         <span className="text-[10px] font-black text-primary uppercase tracking-widest">Upload Image</span>
                      </div>
                    )}
                 </div>
              </div>
              ...
              <div>
                <label className="text-[10px] font-black text-muted uppercase tracking-widest block mb-2">Memory Title / Event Name</label>
                <input 
                  type="text" 
                  required
                  value={form.title}
                  ...
                  placeholder="e.g. Reunion 2024 - Cultural Night"
                />
              </div>
              ...
              >
                {uploading ? <Loader2 className="animate-spin" size={18} /> : <><ImageIconLucide size={18} /> Add Photo</>}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          {loading ? (
            ...
            <div className="bg-white p-20 rounded-[3rem] border-2 border-dashed border-gray-100 text-center">
               <ImageIconLucide size={48} className="mx-auto text-gray-100 mb-6" />
               <p className="text-muted font-bold tracking-tight">Gallery is empty.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {photos.map((p) => (
                <div key={p.id} className="group relative bg-white rounded-[2.5rem] overflow-hidden border border-gray-100 shadow-sm hover:shadow-premium transition-all">
                   ...
                   <div className="p-6">
                      ...
                      <h4 className="font-black text-primary tracking-tight line-clamp-1">{p.title}</h4>
                   </div>
                </div>
              ))}
            </div>
